...
# ------------------ Config ------------------
# ⚠️ Seu token embutido (pode ser sobrescrito via env MAPBOX_TOKEN ou query mapbox_token)
MAPBOX_TOKEN_DEFAULT = "pk.eyJ1Ijoic21hcnRmYXplbmRhcyIsImEiOiJjbHVia3R0djEwajg5MmpxdHl6Y3I0cmI5In0.j6uodlMtsxPm2UpJGzYZsQ"
...
MAPBOX_TOKEN_ENV = os.getenv("MAPBOX_TOKEN", MAPBOX_TOKEN_DEFAULT)
...
DEFAULT_POLY_ALPHA   = float(os.getenv("POLY_ALPHA", "0.28"))    # preenchimento amarelo (opacidade)
...
def _adjust_extent_to_aspect(ext84, target_aspect=TARGET_ASPECT):
    """Ajusta extent WGS84 para que, em WebMercator, width/height == target_aspect."""
...
# ------------------ Mapbox STATIC ------------------
def _fetch_mapbox_static(extent84, width_px, height_px, style_id, token, pin=None):
    """
    Busca uma imagem única via Mapbox Static Images e calcula o extent EPSG:3857.
    style_id: 'mapbox/satellite-v9', 'mapbox/satellite-streets-v12', ou 'user/style'
    """
    if not token:
        raise ValueError("Mapbox token é obrigatório para provider=mapbox_static")
    ...
    # overlay: só usamos o pin no URL (o polígono será desenhado localmente para manter estilo)
    parts = []
    if pin is not None:
        plon, plat, color = pin
        # pin-s cor hex sem '#'
        hexc = color.lstrip('#')
        parts.append(f"pin-s+{hexc}({plon},{plat})")
    ...
    url = (f"{base}{cx},{cy},{zoom}/{width_px}x{height_px}@2x"
    ...
    # extent em 3857 coerente com centro/zoom e tamanho
...
def generate_map():
    try:
        ...
        # transparência do polígono (opacidade = transparencia), mantém compat. com poly_alpha
        ...
        pin_lat = _parse_num(lat_raw)
        pin_lon = _parse_num(lon_raw)
        pin_zoom = int(pin_zoom) if pin_zoom not in (None, "") else None
        ...
        # label dentro do polígono continua "CAR: <cod>" quando existir no KML
        label_text = f"CAR: {cod}" if cod else "CAR"

        # ===== Figura 4:3 (sem bordas) =====
        fig = Figure(figsize=(out_w / dpi, out_h / dpi), dpi=dpi)  # 4:3 perfeito
        ...
        if geom is not None:
            ...
            return jsonify({"error": "Forneça KML em JSON['data'] ou 'latitude' + 'longitude' na query."}), 400

        # ===== Fundo: Mapbox Static ou tiles (fallback) =====
        provider_name = "OpenStreetMap"
        if provider.lower().startswith("mapbox_static"):
            style_id = q.get("style", "mapbox/satellite-v9")
            pin_tuple = None
            if pin_lat is not None and pin_lon is not None:
                pin_tuple = (pin_lon, pin_lat, pin_color)

            ax = fig.add_axes([0, 0, 1, 1], projection=ccrs.epsg(3857))
            img_bg, wm_extent, provider_name = _fetch_mapbox_static(
                extent84, out_w, out_h, style_id, mapbox_token, pin=pin_tuple
            )
            ...
            # caminho antigo (XYZ tiles), caso queira usar google_hybrid, etc.
            if HAS_IMGTILES:
        ...
        # ocultar contornos/axes
        try:
            ax.set_axis_off()
        except Exception:
            pass

        # ===== Máscara externa escura (não afeta a logo) =====
        if geom is not None:
            try:
        ...
        # ===== Rótulo CAR (dentro) quando cod no KML existir =====
        ...
        # ===== Título superior do CAR (opcional via query ?car=...) =====
        if car_title:
        ...
        # Logo no topo-direito, acima da máscara (zorder muito alto)
